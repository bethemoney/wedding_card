...
export default function Footer() {
  const copyAddress = () => {
    const address = window.location.href; // Get the current page URL
    navigator.clipboard.writeText(address).then(() => {
      alert('주소가 복사되었습니다!'); // Alert the user that the address has been copied
    }).catch(err => {
    ...
    <footer className="bg-gray-100 py-4 pb-10">
      <div className="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 py-4">
        <div className="text-center text-gray-800">
          ...
          <br />
          <p>© 디자인,개발</p>
          <Link className='underline' href="https://github.com/bethemoney/wedding_card">Github - Bethemoney</Link>
        </div>
      </div>
    </footer>
  );
